...
\vspace{0.5cm}
As an example of the need for such a machine, see Figure \ref{usbisolator}. I needed to make a USB isolator (to make USB connections to devices with non-isolated
power supplies), which uses an ADUM3160 SOIC-28 part (See full schematic, Figure \ref{usbisolatorschem}). Without an automated method of board manufacture, I hand drew
traces in etch-resist on each side of a double-sided board, and manually soldered all SMD parts. This was slow, and as figure \ref{usbisolator} shows, does not give
neat results.

\section{Terminology}

In this report, the following terminology is assumed:

\begin{itemize} \itemsep0em
	\item	PCB: Printed circuit board. A circuit board made of an insulating sheet covered in conductive traces to which components are soldered. FR-4 is the common grade
			of glass-reinforced epoxy used in most PCBs, and as such is the only material covered by this project.
	\item	SMD: Surface mount device. A component with no through-hole leads which is designed to be soldered to a PCB. In some cases the components are glued to the board
			for strength.
	\item	THT: Through-hole technology. An older technology (compared to SMD) where component leads are inserted into holes in a PCB. Still used in cases where the superior 
...
In order to characterise the accuracy of the machine, it is important to have a consistent measuring regime. I am using a dial gauge to measure
...
\begin{itemize} \itemsep0em
	...
	\item	Rotation: The drawer slides do not elimanate rotation completely, and this error dominates. Rotation is relative to the small ball bearing array which slides along
	...
	\item	Motion parallel to the axis direction. This error is mostly caused by flex in the stepper motor mounts, the motor->threaded rod couplers and the anti-backlash nuts.
